...
function ScriptBotCore() {
	...
	this.initialize = function(nick, ircServer, ircChannel) {
		...
		this.setName(nick);
		this.setLogin(nick);
		this.setVersion("@see http://scriptbot.asmcbain.net for the latest source");
	...
	this.registerPluginInfo = function(plugin, handler, id) {
		if(getUtil().isUndefined(plugin) || getUtil().isUndefined(plugin)) return;
		if(!(handler instanceof Function)) return;
	...
	this.unregisterPlugin = function(plugin, info) {
		if(getUtil().isUndefined(plugin)) return;

		var somethingRemoved = false;

		// "remove" all associated plugins from unrestricted-handlers.
		for(var j in unrestrictedHandlers) {

			var keep = [];
			var event = unrestrictedHandlers[j];
			...
			unrestrictedHandlers[j] = keep;
		}

		// "remove" all associated plugins from restricted-handlers.
		for(var j in restrictedHandlers) {

			var keep = [];
			var event = restrictedHandlers[j];
			for(var i = 0; i < event.length; i++) {
			...
			// Truly remove items we got rid of above.
			keep["unhandled"] =  restrictedHandlers[j]["unhandled"];
			restrictedHandlers[j] = keep;
		}
	...
	this.unregisterPluginByEvent = function(event, id, unrestricted) {
		...
		if(getUtil().isUndefined(event) || getUtil().isUndefined(id)) return;

		var keep = [];
		var somethingRemoved = false;

		// "remove" all associated plugins.
		var list = (getUtil().isTrue(unrestricted))? unrestrictedHandlers[event] : restrictedHandlers[event];
		if(list) {
		...
		// Truly remove items we got rid of above.
		if(getUtil().isTrue(unrestricted)) {
	...
	this.unregisterUnhandledEventPlugin = function(event) {
		...
		if(getUtil().isUndefined(event)) return;
...
/*	this.onPart = function(channel, sender, login, hostname, message) {
	...
	this.onTopic = function(channel, topic, setBy, date, changed) {
		this.fireEvent(Event.TOPIC, [channel+"", topic+"", setBy+"", new Date(date), getUtil().isTrue(changed)]);
	}
...
var config = JSON.parse(IO.readFile("/root/NetBeansProjects/ScriptBotGUI/ScriptBot/src/config.txt"));
